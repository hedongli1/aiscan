...
export const RULES = [
  ...
  // ── 注入类 ────────────────────────────────────────────
  {
    ...
    regex: /\`[^`]*(?:SELECT|INSERT|UPDATE|DELETE)[^`]*\$\{|\b(?:SELECT|INSERT|UPDATE|DELETE)\b[^;\n]{0,80}\$\{|['"](?:SELECT|INSERT|UPDATE|DELETE)[^'"]{0,80}['"]\s*\+/i,
  ...
  // ── XSS / 输出编码 ────────────────────────────────────
  {
    ...
    regex: /\b(?:innerHTML|outerHTML|document\.write)\s*[=\(][\s\S]{0,120}(?:req|query|params|input|data|userInput|location|document\.location|window\.location)/i,
  ...
  // ── 加密与配置 ────────────────────────────────────────
  {
    ...
    regex: /\b(?:createHash|md5|sha1)\s*\(\s*['"](?:md5|sha1)['"]/i,
  ...
  // ── 依赖与权限 ────────────────────────────────────────
  {
    ...
    regex: /["'](?:dependencies|devDependencies|peerDependencies)["']\s*:\s*\{[\s\S]{0,2000}?["'][a-z0-9-]+["']\s*:\s*["']\^/,
    message: '依赖未锁死版本',
