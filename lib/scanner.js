...
// 默认从当前工作目录解析；也可通过 --ignore-file 指定（action 场景用仓库根目录的）
async function loadIgnoreList(ignoreFilePath) {
  const ignore = new Set();
  let text = null;
  if (ignoreFilePath) {
    try {
      text = await fs.readFile(ignoreFilePath, 'utf8');
    } catch {
      // 指定文件不存在则回退到 cwd 查找
    }
  }
  if (text === null) {
    try {
      text = await fs.readFile(path.join(process.cwd(), '.aiscanignore'), 'utf8');
    } catch {
      return ignore; // 无 .aiscanignore 属正常
    }
  }
...
export async function scanDirectory(rootDir, options = {}) {
  ...
  const findings = [];
  const ignoreList = await loadIgnoreList(options.ignoreFile);

  for (const file of files) {
    const ext = path.extname(file).toLowerCase();
    if (SKIP_EXTS.has(ext)) continue;

    // 单文件输入时 relPath 用文件名，目录输入时用相对路径（统一用绝对路径计算）
    let relPath;
    try {
      const st = await fs.stat(rootDir);
      const absRoot = path.resolve(rootDir);
      relPath = st.isFile() ? path.basename(absRoot) : path.relative(absRoot, path.resolve(file));
    } catch {
      ...
      for (const { token, start } of extractTokenCandidates(text)) {
        if (isLikelySecret(token)) {
          ...
          findings.push({
            ...
            category: 'secret',
            title: '疑似高熵 Token / 密钥',
            message: `检测到高熵令牌（熵 ${shannonOf(token)}，置信度 ${confidence}%）`,
            file: relPath,
            line,
            col,
