...
// 从当前工作目录解析，这样无论扫描哪个子目录都能应用项目级忽略规则
async function loadIgnoreList() {
  const ignore = new Set();
  try {
    const text = await fs.readFile(path.join(process.cwd(), '.aiscanignore'), 'utf8');
...
function isIgnored(relPath, ignoreList) {
  ...
  for (const pat of ignoreList) {
    ...
    // 完全匹配，或 basename 匹配（允许 lib/rules/index.js 匹配 rules/index.js）
    if (norm === p) return true;
    if (norm.startsWith(p + '/')) return true;
    if (norm.endsWith('/' + p)) return true;
    if (norm.endsWith('/' + p.split('/').pop())) return true;
  }
  return false;
}
