...
const TEST_DIR_RE = /(^|\/)(tests?|__tests__|spec|fixtures?|example[^/]*|\.github\/workflows|docs?|demo[^/]*)\//i;
  ...
  for (const file of files) {
    ...
    for (const rule of ALL_RULES) {
      ...
      while ((m = re.exec(text)) !== null) {
        ...
        const severity = inTestDir && rule.category === 'secret' ? 'low' : rule.severity;
        findings.push({
          ruleId: rule.id,
          severity,
          category: rule.category,
          title: rule.title,
          message: inTestDir && rule.category === 'secret' ? `${rule.message}（测试目录，已降权）` : rule.message,
          file: relPath,
          line,
          col,
